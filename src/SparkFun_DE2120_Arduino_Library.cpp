...
boolean DE2120::begin(uint8_t baud)
{
  serial.begin(baud);
...
//
boolean DE2120::isConnected()
{
  if(getVersion() != 0x15) return(true); //
  return (false);
}

//
uint8_t DE2120::getVersion()
{
...
// 
void DE2120::sendCommand(char* cmd, char* arg)
{
  char start[] = "^_^";
  char end[] = ".";
